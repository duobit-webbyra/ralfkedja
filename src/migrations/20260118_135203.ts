...
export async function up({ db, payload, req }: MigrateUpArgs): Promise<void> {
  ...
  await db.run(sql`CREATE TABLE \`subscribers_rels\` (
  	\`id\` integer PRIMARY KEY NOT NULL,
  	\`order\` integer,
  	...
  	FOREIGN KEY (\`subscriber_categories_id\`) REFERENCES \`subscriber_categories\`(\`id\`) ON UPDATE no action ON DELETE cascade
  );
  `)
  await db.run(sql`CREATE INDEX \`subscribers_rels_order_idx\` ON \`subscribers_rels\` (\`order\`);`)
  await db.run(sql`CREATE INDEX \`subscribers_rels_parent_idx\` ON \`subscribers_rels\` (\`parent_id\`);`)
  await db.run(sql`CREATE INDEX \`subscribers_rels_path_idx\` ON \`subscribers_rels\` (\`path\`);`)
  await db.run(sql`CREATE INDEX \`subscribers_rels_subscriber_categories_id_idx\` ON \`subscribers_rels\` (\`subscriber_categories_id\`);`)
  await db.run(sql`CREATE TABLE \`subscriber_categories\` (
  	\`id\` integer PRIMARY KEY NOT NULL,
  	\`name\` text NOT NULL,
  	\`slug\` text NOT NULL,
  	\`description\` text,
  ...
  `)
  await db.run(sql`CREATE UNIQUE INDEX \`subscriber_categories_name_idx\` ON \`subscriber_categories\` (\`name\`);`)
  await db.run(sql`CREATE UNIQUE INDEX \`subscriber_categories_slug_idx\` ON \`subscriber_categories\` (\`slug\`);`)
  await db.run(sql`CREATE INDEX \`subscriber_categories_updated_at_idx\` ON \`subscriber_categories\` (\`updated_at\`);`)
  await db.run(sql`CREATE INDEX \`subscriber_categories_created_at_idx\` ON \`subscriber_categories\` (\`created_at\`);`)
  ...
  await db.run(sql`CREATE TABLE \`newsletters_rels\` (
  	\`id\` integer PRIMARY KEY NOT NULL,
  	\`order\` integer,
  	\`parent_id\` integer NOT NULL,
  	\`path\` text NOT NULL,
  	\`subscriber_categories_id\` integer,
  	FOREIGN KEY (\`parent_id\`) REFERENCES \`newsletters\`(\`id\`) ON UPDATE no action ON DELETE cascade,
  	FOREIGN KEY (\`subscriber_categories_id\`) REFERENCES \`subscriber_categories\`(\`id\`) ON UPDATE no action ON DELETE cascade
  );
  `)
  await db.run(sql`CREATE INDEX \`newsletters_rels_order_idx\` ON \`newsletters_rels\` (\`order\`);`)
  await db.run(sql`CREATE INDEX \`newsletters_rels_parent_idx\` ON \`newsletters_rels\` (\`parent_id\`);`)
  await db.run(sql`CREATE INDEX \`newsletters_rels_path_idx\` ON \`newsletters_rels\` (\`path\`);`)
  await db.run(sql`CREATE INDEX \`newsletters_rels_subscriber_categories_id_idx\` ON \`newsletters_rels\` (\`subscriber_categories_id\`);`)
  ...
  await db.run(sql`CREATE INDEX \`payload_locked_documents_rels_subscriber_categories_id_idx\` ON \`payload_locked_documents_rels\` (\`subscriber_categories_id\`);`)
...
export async function down({ db, payload, req }: MigrateDownArgs): Promise<void> {
  ...
  await db.run(sql`DROP TABLE \`subscribers_rels\`;`)
  await db.run(sql`DROP TABLE \`subscriber_categories\`;`)
  await db.run(sql`DROP TABLE \`newsletters\`;`)
  await db.run(sql`DROP TABLE \`newsletters_rels\`;`)
